...
void loop() {
 ...
 static unsigned long configHoldStart = 0;
  ...
  switch (currentState) {
    ...
    case WIFI_SLEEP: {
      if (millis() - sleepStart >= 20000) {  // 1 minute sleep
...
void setupOTA() {
  ArduinoOTA.setHostname("Zone3AQITemp");
...
void loadSettings() {
  ...
  if (settings.interval == 0 || settings.interval > 3600) {
    settings.slaveID1 = 7;
    strcpy(settings.token1, "OAfts0KRgCsYHQARSXbj");
    settings.reg1a = 0;
    strcpy(settings.label1a, "PM_2.5");
    settings.reg1b = 1;
    strcpy(settings.label1b, "PM_10");

    settings.slaveID2 = 11;
    strcpy(settings.token2, "EyNtfmYc25sIRg9lAMkZ");
    settings.reg2a = 1;

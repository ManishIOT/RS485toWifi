...
void loop() {
 ...
 static unsigned long configHoldStart = 0;
  ...
  switch (currentState) {
    ...
    case WIFI_SLEEP: {
      if (millis() - sleepStart >= 6000) {  // 1 minute sleep
...
void setupOTA() {
  ArduinoOTA.setHostname("Zone4AQITemp");
...
void loadSettings() {
  ...
  if (settings.interval == 0 || settings.interval > 3600) {
    settings.slaveID1 = 8;
    strcpy(settings.token1, "nEsVgXezd8aHw8gIR69S");
    settings.reg1a = 0;
    strcpy(settings.label1a, "PM_2.5");
    settings.reg1b = 1;
    strcpy(settings.label1b, "PM_10");

    settings.slaveID2 = 12;
    strcpy(settings.token2, "8FWz6nyOTsL8TnaJZtD8");
    settings.reg2a = 1;
